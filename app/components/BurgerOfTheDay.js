...
const burgerOfTheDay = data.burgerOfTheDay;
  ...
  return  (
    <div id="" className="w-full h-full lg:h-96 relative">
      ...
      <div className="w-full absolute inset-0 flex justify-around place-items-center flex-col gap-8 text-white">
        ...
        {/* <Image className="absolute w-full h-full object-cover" src={burgerBg} alt={`Café de la Place de Brocas, Photo de ${burgerOfTheDay.name}`} /> */}
        ...
        {burgerOfTheDay.specialOffer && (
          <div>
            <span className=''>{burgerOfTheDay.specialOffer.offerText}</span>
            {/* <p>Prix de l&apos;offre : {burgerOfTheDay.specialOffer.price}</p> */}
          </div>
        )}
      </div>
    </div>
